    ...
    <#if requiredActions??>
      <ul>
        <#list requiredActions>
          <#items as reqActionItem><li>${msg("requiredAction.${reqActionItem}")}</li>
          </#items>
        </#list>
      </ul>
    </#if>
...
<html>
  <head>
    ...
    <title>${kcSanitize(msg("emailExecuteActionsTitle"), realmName)}</title>
  </head>
  ...
  <body style="-webkit-text-size-adjust: none; box-sizing: border-box; color: #74787E; font-family: Arial, 'Helvetica Neue', Helvetica, sans-serif; height: 100%; line-height: 1.4; margin: 0; width: 100% !important;" bgcolor="#F2F4F6">
    ...
    <span class="preheader" style="${preHeaderStyles}">
      ${kcSanitize(msg("emailVerificationPreheader", realmName))?no_esc}
    </span>
                  ...
                  <tr>
                    <td class="content-cell" style="${baseTableDataPaddedStyles}">
                      ...
                      <p style="${baseParagraphStyles}" align="left">
                        ${kcSanitize(msg("emailExecuteActionsBodyHtml", realmName))?no_esc}
                      </p>

                      <div style="${baseParagraphStyles}" align="left">
                        ${kcSanitize(requiredActionsText)?no_esc}
                      </div>

                      <p style="${baseParagraphStyles}" align="left">
                        ${kcSanitize(msg("emailExecuteActionsBodyContinuedHtml"))?no_esc}
                      </p>

                      <table class="body-action" align="center" width="100%" cellpadding="0" cellspacing="0" style="${nestedTableStyles}">
                        <tr>
                          <td align="center" style="${baseTableDataStyles}">
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="${baseTableStyles}">
                              <tr>
                                <td align="center" style="${baseTableDataStyles}">
                                  <table border="0" cellspacing="0" cellpadding="0" style="${baseTableStyles}">
                                    <tr>
                                      <td style="${baseTableDataStyles}">
                                        <a href="${link}" class="button button--green" target="_blank" style="${baseButtonStyles}">
                                          ${kcSanitize(msg("emailExecuteActionsLinkText"))}
                                        </a>
                                      </td>
                                    </tr>
                                  </table>
                      ...
                      <p style="${baseParagraphStyles}" align="left">
                        ${msg("emailExecuteActionsBodyNotYouHtml")?no_esc}
                      </p>
